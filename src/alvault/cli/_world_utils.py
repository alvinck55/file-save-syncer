...
from alvault.config.manager import _GAME_REGISTRY, GameDefaults, AlvaultConfig, WorldConfig
...
def resolve_world(cfg: AlvaultConfig, world_name: str | None) -> WorldConfig:
    ...
    if not cfg.worlds:
        typer.echo("Error: no worlds configured. Run `alvault add-world <name> <path>`.", err=True)
        raise typer.Exit(1)
